...
| Search prompt, active match, search highlight | `rotide-maintainer` | search caller, prompt API, buffer search helpers, renderer highlight path | `tests/test_input_search.c`, `tests/test_render_terminal.c` | `make`, `make test` |
...
| LSP lifecycle, definition flow, install/task-log UX | `rotide-maintainer` | `src/language/lsp.c`, `src/input/dispatch.c`, `src/input/actions_language.c`, `src/config/lsp_config.c` | `tests/test_lsp.c` | `make`, `make test`, `make test-sanitize` |
...
| File/module ownership cleanup | `rotide-maintainer` | touched module plus nearest owner/caller | matching domain test file | `make`, `make test`, sanitizer when refactor touches sensitive paths |
