...
- R (`.R`, `.r`, plus `.Rprofile`/`.Rhistory` basenames and an `Rscript` shebang matcher) — vendored from `r-lib/tree-sitter-r` (extern parser `tree_sitter_r`, root node `program`). Ships an external `scanner.c` (16 externals for newline/semicolon/raw-string/bracket handling), compiled via the Makefile's `$(wildcard)` scanner rule. Upstream `highlights.scm` and `locals.scm` map cleanly to RotIDE classes and are vendored **as-is** (manifest symbols `r_highlight`, `r_locals`) — like Ruby. The highlight query captures every `(identifier)` as `@variable`, then overrides call heads / definitions as `@function`, parameters as `@variable.parameter`, `::` left-hand sides as `@namespace`, and `#eq?`-gates `return()` to `@keyword`. Its `(ERROR) @error` capture is harmless: `error` maps to no RotIDE highlight class and is skipped. No injections.
