...
- Perl (`.pl`, `.pm`, `.t`, plus extensionless `perl` shebang detection) — vendored from the v1.2.1 source commit deployed to the `release` branch of `tree-sitter-perl/tree-sitter-perl` (extern parser `tree_sitter_perl`, root node `source_file`). Ships an external scanner and helper headers beside `scanner.c`. The refresh path changes the upstream highlight query's `#lua-match?` predicate to standard `#match?`. Its injection query reparses evaluated substitution replacements as Perl (with `injection.include-children` added for RotIDE's range semantics) and dynamically routes heredocs by terminator; unsupported `comment`/`pod` targets are skipped gracefully.
