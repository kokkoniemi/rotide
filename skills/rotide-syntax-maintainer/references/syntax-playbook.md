...
- EJS (`.ejs`) — extern parser `tree_sitter_embedded_template` (shared upstream grammar `tree-sitter-embedded-template`, vendored as `embedded_template/`; the same parser would back ERB/ETLUA when added). Grammar root is `template`. Parser-only — no `scanner.c`, so the Makefile only compiles `parser.c`. Upstream ships per-dialect injection queries (`injections-ejs.scm`, `injections-erb.scm`, `injections-etlua.scm`) plus a single `highlights.scm` that only marks tag delimiters and `(comment_directive)`. **Injection wiring is not implemented for EJS** — the existing `g_html_injection_query_cache` path is HTML-specific (hardcoded JS/CSS injection trees in `editorSyntaxState`); EJS therefore renders only its tag delimiters and comment-directive bodies. Adding HTML+JS injection highlighting for EJS would require a generalized injection trees mechanism in syntax state.
- Regex (`.regex`)
