#include "language/lsp.h"
#include "language/syntax.h"
#include "rotide.h"
#include "workspace/drawer.h"
#include "workspace/drawer_internal.h"
#include "workspace/file_search.h"
#include "workspace/project_search.h"
#include "workspace/tabs.h"

#include <limits.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

enum drawerModeLspGroup {
	EDITOR_DRAWER_LSP_GROUP_PROBLEMS = 0,
	EDITOR_DRAWER_LSP_GROUP_SYMBOLS,
	EDITOR_DRAWER_LSP_GROUP_USAGES,
	EDITOR_DRAWER_LSP_GROUP_COUNT
};

#define EDITOR_DRAWER_LSP_BASE_GROUP_COUNT 2

enum drawerModeLspEntryKind {
	EDITOR_DRAWER_LSP_ENTRY_ROOT = 0,
	EDITOR_DRAWER_LSP_ENTRY_GROUP,
	EDITOR_DRAWER_LSP_ENTRY_PROBLEM,
	EDITOR_DRAWER_LSP_ENTRY_SYMBOL,
	EDITOR_DRAWER_LSP_ENTRY_USAGE,
	EDITOR_DRAWER_LSP_ENTRY_PLACEHOLDER
};

enum drawerModeLspProblemSource {
	EDITOR_DRAWER_LSP_PROBLEM_LSP = 0,
	EDITOR_DRAWER_LSP_PROBLEM_SYNTAX
};

struct drawerModeLspProblem {
	enum drawerModeLspProblemSource source;
	const char *path;
	const char *message;
	int line;
	int character;
	int severity;
};

struct drawerModeLspSymbolEntry {
	const char *name;
	const char *path;
	int kind;
	int line;
	int character;
	int depth;
	int parent_index;
	int is_last_sibling;
};

struct drawerModeLspUsageEntry {
	const char *path;
	const char *display;
	int line;
	int character;
};

struct drawerModeLspLookup {
	enum drawerModeLspEntryKind kind;
	int group_idx;
	int item_idx;
	int item_count;
	int visible_idx;
	int parent_visible_idx;
	int group_visible_idx;
	struct drawerModeLspProblem problem;
	struct drawerModeLspSymbolEntry symbol;
	struct drawerModeLspUsageEntry usage;
};

static const char *g_drawer_mode_lsp_group_names[EDITOR_DRAWER_LSP_GROUP_COUNT] = {
        "Problems", "Symbols", "Usages"};

static int drawerModeLspUsagesActive(void) {
	return E.drawer_usages_symbol != NULL || E.drawer_usages_count > 0;
}

static int drawerModeLspVisibleGroupCount(void) {
	return EDITOR_DRAWER_LSP_BASE_GROUP_COUNT + (drawerModeLspUsagesActive() ? 1 : 0);
}

static unsigned int drawerModeLspAllGroupsMask(void) {
	unsigned int mask = 0;
	for (int i = 0; i < drawerModeLspVisibleGroupCount(); i++) {
		mask |= 1u << (unsigned int)i;
	}
	return mask;
}

static int drawerModeLspGroupExpanded(int group_idx) {
	if (group_idx < 0 || group_idx >= EDITOR_DRAWER_LSP_GROUP_COUNT) {
		return 0;
	}
	return (E.drawer_lsp_expanded & (1u << (unsigned int)group_idx)) != 0;
}

static void drawerModeLspEnsureDefaultExpanded(void) {
	E.drawer_lsp_expanded = drawerModeLspAllGroupsMask();
}

static int drawerModeLspTabHasSyntaxProblem(const struct editorBuffer *tab, int *line_out,
                                            int *character_out) {
	struct editorSyntaxState *state = tab != NULL ? tab->syntax_state : NULL;
	return editorSyntaxStateFirstErrorPosition(state, line_out, character_out);
}

static int drawerModeLspTabProblemCount(int tab_idx) {
	const struct editorBuffer *tab = editorTabBufferHandleAt(tab_idx);
	if (tab == NULL) {
		return 0;
	}
	int count = tab->lsp_diagnostic_count;
	if (drawerModeLspTabHasSyntaxProblem(tab, NULL, NULL)) {
		count++;
	}
	return count;
}

static int drawerModeLspProblemCount(void) {
	int count = 0;
	for (int tab_idx = 0; tab_idx < E.tab_count; tab_idx++) {
		count += drawerModeLspTabProblemCount(tab_idx);
	}
	return count;
}

static int drawerModeLspProblemAt(int problem_idx, struct drawerModeLspProblem *problem_out) {
	if (problem_out == NULL || problem_idx < 0) {
		return 0;
	}

	for (int tab_idx = 0; tab_idx < E.tab_count; tab_idx++) {
		const struct editorBuffer *tab = editorTabBufferHandleAt(tab_idx);
		if (tab == NULL) {
			continue;
		}
		const char *filename = tab->filename;

		int line = 0;
		int character = 0;
		if (drawerModeLspTabHasSyntaxProblem(tab, &line, &character)) {
			if (problem_idx == 0) {
				memset(problem_out, 0, sizeof(*problem_out));
				problem_out->source = EDITOR_DRAWER_LSP_PROBLEM_SYNTAX;
				problem_out->path = filename;
				problem_out->message = "Syntax parse error";
				problem_out->line = line;
				problem_out->character = character;
				problem_out->severity = 1;
				return 1;
			}
			problem_idx--;
		}

		const struct editorLspDiagnostic *diagnostics = tab->lsp_diagnostics;
		int diagnostic_count = tab->lsp_diagnostic_count;
		for (int i = 0; i < diagnostic_count; i++) {
			if (problem_idx == 0) {
				memset(problem_out, 0, sizeof(*problem_out));
				problem_out->source = EDITOR_DRAWER_LSP_PROBLEM_LSP;
				problem_out->path = filename;
				problem_out->message = diagnostics[i].message;
				problem_out->line = diagnostics[i].start_line;
				problem_out->character = diagnostics[i].start_character;
				problem_out->severity = diagnostics[i].severity;
				return 1;
			}
			problem_idx--;
		}
	}

	return 0;
}

static int drawerModeLspSymbolCount(void) {
	return E.lsp_symbol_count > 0 ? E.lsp_symbol_count : 0;
}

static int drawerModeLspSymbolAt(int symbol_idx, struct drawerModeLspSymbolEntry *symbol_out) {
	if (symbol_out == NULL || symbol_idx < 0 || symbol_idx >= E.lsp_symbol_count ||
	    E.lsp_symbols == NULL) {
		return 0;
	}
	const struct editorLspSymbol *src = &E.lsp_symbols[symbol_idx];
	memset(symbol_out, 0, sizeof(*symbol_out));
	symbol_out->name = src->name != NULL ? src->name : "";
	symbol_out->path = E.filename;
	symbol_out->kind = src->kind;
	symbol_out->line = src->line;
	symbol_out->character = src->character;
	symbol_out->depth = src->depth;
	symbol_out->parent_index = src->parent_index;
	symbol_out->is_last_sibling = src->is_last_sibling;
	return 1;
}

static int drawerModeLspUsageCount(void) {
	return E.drawer_usages_count > 0 ? E.drawer_usages_count : 0;
}

static int drawerModeLspUsageAt(int usage_idx, struct drawerModeLspUsageEntry *usage_out) {
	if (usage_out == NULL || usage_idx < 0 || usage_idx >= E.drawer_usages_count ||
	    E.drawer_usages_results == NULL) {
		return 0;
	}
	const struct editorUsageResult *src = &E.drawer_usages_results[usage_idx];
	usage_out->path = src->path;
	usage_out->display = src->display;
	usage_out->line = src->line;
	usage_out->character = src->character;
	return 1;
}

static int drawerModeLspGroupItemCount(int group_idx) {
	if (group_idx == EDITOR_DRAWER_LSP_GROUP_PROBLEMS) {
		return drawerModeLspProblemCount();
	}
	if (group_idx == EDITOR_DRAWER_LSP_GROUP_SYMBOLS) {
		return drawerModeLspSymbolCount();
	}
	if (group_idx == EDITOR_DRAWER_LSP_GROUP_USAGES) {
		return drawerModeLspUsageCount();
	}
	return 0;
}

int editorDrawerLspVisibleCount(void) {
	int count = 1;
	int group_count = drawerModeLspVisibleGroupCount();
	for (int group_idx = 0; group_idx < group_count; group_idx++) {
		count++;
		if (!drawerModeLspGroupExpanded(group_idx)) {
			continue;
		}
		int item_count = drawerModeLspGroupItemCount(group_idx);
		count += item_count > 0 ? item_count : 1;
	}
	return count;
}

static int drawerModeLspLookupByVisibleIndex(int visible_idx,
                                             struct drawerModeLspLookup *lookup_out) {
	if (lookup_out == NULL || visible_idx < 0) {
		return 0;
	}

	memset(lookup_out, 0, sizeof(*lookup_out));
	lookup_out->visible_idx = visible_idx;
	lookup_out->group_idx = -1;
	lookup_out->item_idx = -1;
	lookup_out->parent_visible_idx = -1;
	lookup_out->group_visible_idx = -1;

	if (visible_idx == 0) {
		lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_ROOT;
		return 1;
	}

	int cursor = 1;
	int group_count = drawerModeLspVisibleGroupCount();
	for (int group_idx = 0; group_idx < group_count; group_idx++) {
		int group_visible_idx = cursor;
		int item_count = drawerModeLspGroupItemCount(group_idx);
		if (visible_idx == group_visible_idx) {
			lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_GROUP;
			lookup_out->group_idx = group_idx;
			lookup_out->parent_visible_idx = 0;
			lookup_out->group_visible_idx = group_visible_idx;
			lookup_out->item_count = item_count;
			return 1;
		}
		cursor++;

		if (!drawerModeLspGroupExpanded(group_idx)) {
			continue;
		}

		if (item_count == 0) {
			if (visible_idx == cursor) {
				lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_PLACEHOLDER;
				lookup_out->group_idx = group_idx;
				lookup_out->parent_visible_idx = group_visible_idx;
				lookup_out->group_visible_idx = group_visible_idx;
				return 1;
			}
			cursor++;
			continue;
		}

		for (int item_idx = 0; item_idx < item_count; item_idx++) {
			if (visible_idx == cursor) {
				lookup_out->group_idx = group_idx;
				lookup_out->item_idx = item_idx;
				lookup_out->item_count = item_count;
				lookup_out->group_visible_idx = group_visible_idx;
				if (group_idx == EDITOR_DRAWER_LSP_GROUP_SYMBOLS) {
					lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_SYMBOL;
					if (!drawerModeLspSymbolAt(item_idx, &lookup_out->symbol)) {
						return 0;
					}
					lookup_out->parent_visible_idx =
					        lookup_out->symbol.parent_index >= 0
					                ? group_visible_idx + 1 +
					                          lookup_out->symbol.parent_index
					                : group_visible_idx;
					return 1;
				}
				if (group_idx == EDITOR_DRAWER_LSP_GROUP_USAGES) {
					lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_USAGE;
					lookup_out->parent_visible_idx = group_visible_idx;
					return drawerModeLspUsageAt(item_idx, &lookup_out->usage);
				}
				lookup_out->parent_visible_idx = group_visible_idx;
				lookup_out->kind = EDITOR_DRAWER_LSP_ENTRY_PROBLEM;
				return drawerModeLspProblemAt(item_idx, &lookup_out->problem);
			}
			cursor++;
		}
	}

	return 0;
}

int editorDrawerLspToggle(void) {
	if (E.drawer_mode == EDITOR_DRAWER_MODE_LSP) {
		E.drawer_mode = EDITOR_DRAWER_MODE_TREE;
		E.drawer_selected_index = -1;
		E.drawer_rowoff = 0;
		E.drawer_resize_active = 0;
		E.primary_focus = EDITOR_PRIMARY_FOCUS_DRAWER;
		return 1;
	}

	if (editorFileSearchIsActive()) {
		editorFileSearchExit(1);
	}
	if (editorProjectSearchIsActive()) {
		editorProjectSearchExit(1);
	}
	editorLspRefreshActiveDocumentSymbols();
	drawerModeLspEnsureDefaultExpanded();
	E.drawer_mode = EDITOR_DRAWER_MODE_LSP;
	E.drawer_selected_index = -1;
	E.drawer_rowoff = 0;
	E.drawer_resize_active = 0;
	(void)editorDrawerSetCollapsed(0);
	E.primary_focus = EDITOR_PRIMARY_FOCUS_DRAWER;
	return 1;
}

int editorDrawerLspVisibleEntryView(int visible_idx, struct editorDrawerEntryView *view_out) {
	if (view_out == NULL) {
		return 0;
	}

	static char lsp_name_buf[PATH_MAX + 128];
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(visible_idx, &lookup)) {
		return 0;
	}

	memset(view_out, 0, sizeof(*view_out));
	view_out->is_selected = visible_idx == E.drawer_selected_index;
	view_out->parent_visible_idx = lookup.parent_visible_idx;
	view_out->line = lookup.problem.line;
	view_out->character = lookup.problem.character;
	switch (lookup.kind) {
		case EDITOR_DRAWER_LSP_ENTRY_ROOT:
			view_out->name = "LSP";
			view_out->depth = 0;
			view_out->is_dir = 1;
			view_out->is_expanded = 1;
			view_out->is_root = 1;
			view_out->is_last_sibling = 1;
			return 1;
		case EDITOR_DRAWER_LSP_ENTRY_GROUP:
			if (lookup.group_idx == EDITOR_DRAWER_LSP_GROUP_PROBLEMS) {
				(void)snprintf(lsp_name_buf, sizeof(lsp_name_buf), "Problems (%d)",
				               lookup.item_count);
				view_out->name = lsp_name_buf;
			} else if (lookup.group_idx == EDITOR_DRAWER_LSP_GROUP_USAGES) {
				const char *symbol = E.drawer_usages_symbol;
				if (symbol != NULL && symbol[0] != '\0') {
					(void)snprintf(lsp_name_buf, sizeof(lsp_name_buf),
					               "Usages of `%s` (%d)", symbol,
					               lookup.item_count);
				} else {
					(void)snprintf(lsp_name_buf, sizeof(lsp_name_buf),
					               "Usages (%d)", lookup.item_count);
				}
				view_out->name = lsp_name_buf;
			} else {
				view_out->name = g_drawer_mode_lsp_group_names[lookup.group_idx];
			}
			view_out->depth = 1;
			view_out->is_dir = 1;
			view_out->is_expanded = drawerModeLspGroupExpanded(lookup.group_idx);
			view_out->is_last_sibling =
			        lookup.group_idx == drawerModeLspVisibleGroupCount() - 1;
			return 1;
		case EDITOR_DRAWER_LSP_ENTRY_PROBLEM: {
			const char *path = lookup.problem.path != NULL ? lookup.problem.path : "";
			const char *slash = strrchr(path, '/');
			const char *base = slash != NULL ? slash + 1 : path;
			const char *message =
			        lookup.problem.message != NULL ? lookup.problem.message : "";
			const char *kind = lookup.problem.source == EDITOR_DRAWER_LSP_PROBLEM_SYNTAX
			                           ? "Syntax"
			                           : "Info";
			if (lookup.problem.source == EDITOR_DRAWER_LSP_PROBLEM_LSP) {
				if (lookup.problem.severity == 1) {
					kind = "Error";
				} else if (lookup.problem.severity == 2) {
					kind = "Warning";
				}
			}
			(void)snprintf(lsp_name_buf, sizeof(lsp_name_buf), "%s %s:%d:%d %s", kind,
			               base != NULL && base[0] != '\0' ? base : "(untitled)",
			               lookup.problem.line + 1, lookup.problem.character + 1,
			               message);
			view_out->name = lsp_name_buf;
			view_out->path = lookup.problem.path;
			view_out->depth = 2;
			view_out->is_last_sibling = lookup.item_idx == lookup.item_count - 1;
			view_out->lsp_problem_severity = lookup.problem.severity;
			view_out->lsp_problem_kind_len = (int)strlen(kind);
			return 1;
		}
		case EDITOR_DRAWER_LSP_ENTRY_SYMBOL: {
			const char *symbol_name =
			        lookup.symbol.name != NULL && lookup.symbol.name[0] != '\0'
			                ? lookup.symbol.name
			                : "(unnamed)";
			(void)snprintf(lsp_name_buf, sizeof(lsp_name_buf), "%s %s:%d",
			               editorLspSymbolKindLabel(lookup.symbol.kind), symbol_name,
			               lookup.symbol.line + 1);
			view_out->name = lsp_name_buf;
			view_out->path = lookup.symbol.path;
			view_out->line = lookup.symbol.line;
			view_out->character = lookup.symbol.character;
			view_out->depth = 2 + lookup.symbol.depth;
			view_out->is_last_sibling = lookup.symbol.is_last_sibling;
			return 1;
		}
		case EDITOR_DRAWER_LSP_ENTRY_USAGE:
			view_out->name = lookup.usage.display != NULL ? lookup.usage.display : "";
			view_out->path = lookup.usage.path;
			view_out->line = lookup.usage.line;
			view_out->character = lookup.usage.character;
			view_out->depth = 2;
			view_out->is_last_sibling = lookup.item_idx == lookup.item_count - 1;
			return 1;
		case EDITOR_DRAWER_LSP_ENTRY_PLACEHOLDER:
			view_out->name = lookup.group_idx == EDITOR_DRAWER_LSP_GROUP_SYMBOLS
			                         ? "(symbols not loaded yet)"
			                 : lookup.group_idx == EDITOR_DRAWER_LSP_GROUP_USAGES
			                         ? "(no usages)"
			                         : "(none)";
			view_out->depth = 2;
			view_out->is_placeholder = 1;
			view_out->is_last_sibling = 1;
			return 1;
		default:
			return 0;
	}
}

int editorDrawerLspExpandSelection(int viewport_rows) {
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(E.drawer_selected_index, &lookup)) {
		return 0;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_ROOT) {
		editorDrawerClampSelectionAndScroll(viewport_rows);
		return 0;
	}
	if (lookup.kind != EDITOR_DRAWER_LSP_ENTRY_GROUP ||
	    drawerModeLspGroupExpanded(lookup.group_idx)) {
		return 0;
	}
	E.drawer_lsp_expanded |= 1u << (unsigned int)lookup.group_idx;
	editorDrawerClampSelectionAndScroll(viewport_rows);
	return 1;
}

int editorDrawerLspCollapseSelection(int viewport_rows) {
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(E.drawer_selected_index, &lookup)) {
		return 0;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_ROOT) {
		editorDrawerClampSelectionAndScroll(viewport_rows);
		return 0;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_GROUP) {
		if (!drawerModeLspGroupExpanded(lookup.group_idx)) {
			return 0;
		}
		E.drawer_lsp_expanded &= ~(1u << (unsigned int)lookup.group_idx);
		editorDrawerClampSelectionAndScroll(viewport_rows);
		return 1;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_PROBLEM ||
	    lookup.kind == EDITOR_DRAWER_LSP_ENTRY_USAGE ||
	    lookup.kind == EDITOR_DRAWER_LSP_ENTRY_PLACEHOLDER) {
		E.drawer_selected_index = lookup.group_visible_idx;
		editorDrawerClampSelectionAndScroll(viewport_rows);
		return 1;
	}
	return 0;
}

int editorDrawerLspToggleSelectionExpanded(int viewport_rows) {
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(E.drawer_selected_index, &lookup)) {
		return 0;
	}
	if (lookup.kind != EDITOR_DRAWER_LSP_ENTRY_GROUP) {
		return 0;
	}
	E.drawer_lsp_expanded ^= 1u << (unsigned int)lookup.group_idx;
	editorDrawerClampSelectionAndScroll(viewport_rows);
	return 1;
}

int editorDrawerLspSelectedIsDirectory(void) {
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(E.drawer_selected_index, &lookup)) {
		return 0;
	}
	return lookup.kind == EDITOR_DRAWER_LSP_ENTRY_ROOT ||
	       lookup.kind == EDITOR_DRAWER_LSP_ENTRY_GROUP;
}

int editorDrawerSelectedLspLocation(const char **path_out, int *line_out, int *character_out) {
	if (path_out == NULL || line_out == NULL || character_out == NULL ||
	    E.drawer_mode != EDITOR_DRAWER_MODE_LSP) {
		return 0;
	}
	struct drawerModeLspLookup lookup;
	if (!drawerModeLspLookupByVisibleIndex(E.drawer_selected_index, &lookup)) {
		return 0;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_PROBLEM) {
		if (lookup.problem.path == NULL || lookup.problem.path[0] == '\0') {
			return 0;
		}
		*path_out = lookup.problem.path;
		*line_out = lookup.problem.line;
		*character_out = lookup.problem.character;
		return 1;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_SYMBOL) {
		if (lookup.symbol.path == NULL || lookup.symbol.path[0] == '\0') {
			return 0;
		}
		*path_out = lookup.symbol.path;
		*line_out = lookup.symbol.line;
		*character_out = lookup.symbol.character;
		return 1;
	}
	if (lookup.kind == EDITOR_DRAWER_LSP_ENTRY_USAGE) {
		if (lookup.usage.path == NULL || lookup.usage.path[0] == '\0') {
			return 0;
		}
		*path_out = lookup.usage.path;
		*line_out = lookup.usage.line;
		*character_out = lookup.usage.character;
		return 1;
	}
	return 0;
}

static const char *drawerModeLspUsagesDisplayPath(const char *path) {
	const char *root = editorDrawerRootPath();
	if (root != NULL && root[0] != '\0' && path != NULL) {
		size_t root_len = strlen(root);
		if (strncmp(path, root, root_len) == 0 && path[root_len] == '/') {
			return path + root_len + 1;
		}
	}
	return path != NULL ? path : "";
}

static int drawerModeLspUsageCompare(const void *a, const void *b) {
	const struct editorUsageResult *ua = a;
	const struct editorUsageResult *ub = b;
	int cmp = strcmp(ua->path != NULL ? ua->path : "", ub->path != NULL ? ub->path : "");
	if (cmp != 0) {
		return cmp;
	}
	if (ua->line != ub->line) {
		return ua->line < ub->line ? -1 : 1;
	}
	if (ua->character != ub->character) {
		return ua->character < ub->character ? -1 : 1;
	}
	return 0;
}

void editorDrawerLspUsagesFree(void) {
	if (E.drawer_usages_results != NULL) {
		for (int i = 0; i < E.drawer_usages_count; i++) {
			free(E.drawer_usages_results[i].path);
			free(E.drawer_usages_results[i].display);
		}
		free(E.drawer_usages_results);
		E.drawer_usages_results = NULL;
	}
	E.drawer_usages_count = 0;
	E.drawer_usages_capacity = 0;
	free(E.drawer_usages_symbol);
	E.drawer_usages_symbol = NULL;
	E.drawer_lsp_expanded &= ~(1u << (unsigned int)EDITOR_DRAWER_LSP_GROUP_USAGES);
}

static int drawerModeLspUsagesPopulate(const struct editorLspLocation *locations, int count,
                                       const char *symbol) {
	editorDrawerLspUsagesFree();
	if (locations == NULL || count <= 0) {
		return 0;
	}

	struct editorUsageResult *results = calloc((size_t)count, sizeof(*results));
	if (results == NULL) {
		return 0;
	}

	int stored = 0;
	for (int i = 0; i < count; i++) {
		const char *path = locations[i].path;
		if (path == NULL || path[0] == '\0') {
			continue;
		}
		char display[PATH_MAX + 64];
		(void)snprintf(display, sizeof(display), "%s:%d:%d",
		               drawerModeLspUsagesDisplayPath(path), locations[i].line + 1,
		               locations[i].character + 1);
		results[stored].path = strdup(path);
		results[stored].display = strdup(display);
		if (results[stored].path == NULL || results[stored].display == NULL) {
			free(results[stored].path);
			free(results[stored].display);
			for (int j = 0; j < stored; j++) {
				free(results[j].path);
				free(results[j].display);
			}
			free(results);
			return 0;
		}
		results[stored].line = locations[i].line;
		results[stored].character = locations[i].character;
		stored++;
	}

	if (stored == 0) {
		free(results);
		return 0;
	}

	qsort(results, (size_t)stored, sizeof(*results), drawerModeLspUsageCompare);
	E.drawer_usages_results = results;
	E.drawer_usages_count = stored;
	E.drawer_usages_capacity = count;
	E.drawer_usages_symbol = symbol != NULL ? strdup(symbol) : NULL;
	return 1;
}

static int drawerModeLspUsagesGroupVisibleIndex(void) {
	int cursor = 1;
	int group_count = drawerModeLspVisibleGroupCount();
	for (int group_idx = 0; group_idx < group_count; group_idx++) {
		if (group_idx == EDITOR_DRAWER_LSP_GROUP_USAGES) {
			return cursor;
		}
		cursor++;
		if (drawerModeLspGroupExpanded(group_idx)) {
			int item_count = drawerModeLspGroupItemCount(group_idx);
			cursor += item_count > 0 ? item_count : 1;
		}
	}
	return -1;
}

int editorDrawerLspShowUsages(const struct editorLspLocation *locations, int count,
                              const char *symbol) {
	if (!drawerModeLspUsagesPopulate(locations, count, symbol)) {
		return 0;
	}

	if (editorFileSearchIsActive()) {
		editorFileSearchExit(1);
	}
	if (editorProjectSearchIsActive()) {
		editorProjectSearchExit(1);
	}
	editorLspRefreshActiveDocumentSymbols();
	drawerModeLspEnsureDefaultExpanded();
	E.drawer_mode = EDITOR_DRAWER_MODE_LSP;
	E.drawer_rowoff = 0;
	E.drawer_resize_active = 0;
	(void)editorDrawerSetCollapsed(0);
	E.primary_focus = EDITOR_PRIMARY_FOCUS_DRAWER;

	int group_visible_idx = drawerModeLspUsagesGroupVisibleIndex();
	E.drawer_selected_index = group_visible_idx >= 0 && E.drawer_usages_count > 0
	                                  ? group_visible_idx + 1
	                                  : group_visible_idx;
	/* Earlier groups can push the selected usage beyond the viewport. */
	editorDrawerClampSelectionAndScroll(E.window_rows);
	return 1;
}
