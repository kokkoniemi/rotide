...
static const char *textSourceDocumentRead(const struct editorTextSource *source, size_t byte_index,
                                          uint32_t *bytes_read) {
...
char *editorDupActiveTextSource(size_t *len_out) {
	...
	g_text_source_active_dup_count++;
	if (len_out != NULL) {
		*len_out = 0;
	}
...
void editorActiveTextSourceBuildTestResetCount(void) {
	g_text_source_active_build_count = 0;
}

int editorActiveTextSourceBuildTestCount(void) {
	return g_text_source_active_build_count;
}

void editorActiveTextSourceDupTestResetCount(void) {
	g_text_source_active_dup_count = 0;
}

int editorActiveTextSourceDupTestCount(void) {
	return g_text_source_active_dup_count;
}
