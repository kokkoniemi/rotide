...
Start with Phase 1 (runner ergonomics + reset-state validator) as a single
PR, plus the `tests/QUARANTINE.md` skeleton from the cross-cutting section.
This is small, unblocks flake hunting and incident response immediately,
gives `--shuffle` something it can be trusted on, and is a hard prerequisite
for the seeded property tests in Phase 3. Phase 2 (parallel suites + crash
artifacts) is the natural follow-up before the suite count grows further
with Phases 3–5. The TSan target and determinism gate from the cross-cutting
section can land in parallel with Phase 1; they don't depend on it.
