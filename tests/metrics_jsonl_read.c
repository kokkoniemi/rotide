...
const char *editorMetricsKindName(enum editorMetricsKind k) {
	switch (k) {
	case EDITOR_METRICS_KIND_TEST_RUN: return "test_run";
	case EDITOR_METRICS_KIND_BENCH:    return "bench";
	case EDITOR_METRICS_KIND_FUZZ:     return "fuzz";
...
static int row_capacity_grow(struct editorMetricsRow **rows, int *cap) {
	...
	struct editorMetricsRow *grown = (struct editorMetricsRow *)realloc(
		*rows, (size_t)new_cap * sizeof(**rows));
	if (grown == NULL) {
		return -1;
	}
	*rows = grown;
