...
static struct editorMetricsRow **collectGroup(const struct editorMetricsRow *rows, int count,
	...
	*out_count = 0;
	...
	for (int i = 0; i < count; i++) {
		...
		if (group_key != NULL) {
			const char *rk =
			        kind == EDITOR_METRICS_KIND_FUZZ ? r->fuzz_target : r->bench_name;
...
static const char **collectGroupKeys(const struct editorMetricsRow *rows, int count,
	...
	*out_count = 0;
	const char **keys = (const char **)malloc((size_t)(count > 0 ? count : 1) * sizeof(*keys));
	if (keys == NULL) {
		return NULL;
	}
	int n = 0;
...
static void summarizeGroup(const struct editorMetricsRow *rows, int count,
	...
	int group_count = 0;
	struct editorMetricsRow **group =
	        collectGroup(rows, count, kind, group_key, opts, &group_count);
	...
	if (group_key != NULL) {
		(void)fprintf(out, "== %s: %s (%d row%s shown", editorMetricsKindName(kind), group_key,
		        shown, shown == 1 ? "" : "s");
	} else {
...
int editorMetricsCmdSummary(const struct editorMetricsRow *rows, int count,
	...
	if (opts->kind_filter == EDITOR_METRICS_KIND_UNKNOWN ||
		...
		int n = 0;
		const char **keys =
		        collectGroupKeys(rows, count, EDITOR_METRICS_KIND_BENCH, opts, &n);
		for (int i = 0; i < n; i++) {
	...
	if (opts->kind_filter == EDITOR_METRICS_KIND_UNKNOWN ||
		...
		int n = 0;
		const char **keys =
		        collectGroupKeys(rows, count, EDITOR_METRICS_KIND_FUZZ, opts, &n);
		for (int i = 0; i < n; i++) {
...
int editorMetricsCmdCheckFuzzStale(const struct editorMetricsRow *rows, int count,
	...
	long long window_hours = opts->window_hours > 0 ? opts->window_hours : DEFAULT_WINDOW_HOURS;
	...
	for (int i = 0; i < n_keys; i++) {
		...
		struct editorMetricsRow **group = collectGroup(
		        rows, count, EDITOR_METRICS_KIND_FUZZ, keys[i], &effective, &gc);
		if (group == NULL || gc == 0) {
			free(group);
			continue;
		}
...
int editorMetricsCmdCheckBenchRegression(const struct editorMetricsRow *rows, int count,
	...
	double factor =
	        opts->regression_factor > 0 ? opts->regression_factor : DEFAULT_REGRESSION_FACTOR;
