...
static int editorTestSaveSyscallsFailureProbe(enum editorSaveSyscallOp op, int *failure_errno) {
	switch (op) {
		case EDITOR_SAVE_SYSCALL_RENAME:
			...
			if (editor_save_syscall_test_state.fail_rename_on_call > 0 &&
				...
				if (failure_errno != NULL) {
					*failure_errno =
					        editor_save_syscall_test_state.fail_rename_errno;
				}
				return 1;
			}
			return 0;
		case EDITOR_SAVE_SYSCALL_FSYNC:
			...
			if (editor_save_syscall_test_state.fail_fsync_on_call > 0 &&
				...
				if (failure_errno != NULL) {
					*failure_errno =
					        editor_save_syscall_test_state.fail_fsync_errno;
				}
				return 1;
			}
			return 0;
		case EDITOR_SAVE_SYSCALL_OPEN_DIR:
			...
			if (editor_save_syscall_test_state.fail_open_dir_on_call > 0 &&
				...
				if (failure_errno != NULL) {
					*failure_errno =
					        editor_save_syscall_test_state.fail_open_dir_errno;
				}
				return 1;
			}
			return 0;
		case EDITOR_SAVE_SYSCALL_CLOSE:
			...
			if (editor_save_syscall_test_state.fail_close_on_call > 0 &&
				...
				if (failure_errno != NULL) {
					*failure_errno =
					        editor_save_syscall_test_state.fail_close_errno;
				}
				return 1;
			}
			return 0;
		case EDITOR_SAVE_SYSCALL_UNLINK:
			...
			if (editor_save_syscall_test_state.fail_unlink_on_call > 0 &&
				...
				if (failure_errno != NULL) {
					*failure_errno =
					        editor_save_syscall_test_state.fail_unlink_errno;
				}
				return 1;
			}
			return 0;
