...
static int test_editor_refresh_screen_applies_ubuntu_theme(void) {
	char path[] = "/tmp/rotide-test-syntax-highlight-ubuntu-XXXXXX.c";
	ASSERT_TRUE(write_fixture_to_temp_path(path, 2,
			"tests/syntax/supported/c/highlight.c"));

	editorOpen(path);
	ASSERT_TRUE(editorThemeInitBuiltin(&E.theme, "ubuntu"));
	E.window_rows = 8;
	E.window_cols = 100;
	E.cy = 0;
	E.cx = 0;
	...
	char *output = refresh_screen_and_capture(&output_len);
	ASSERT_TRUE(output != NULL);
	ASSERT_TRUE(strstr(output, "\x1b]12;rgb:f7/f7/f7\a") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[48;2;44;0;30m") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[38;2;214;188;212mmain") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[38;2;174;167;159m// comment") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[38;2;191;169;185m\"txt\"") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[38;2;233;84;32mreturn") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[38;2;187;144;183m42") != NULL);
	ASSERT_TRUE(strstr(output, "\x1b[48;2;58;8;39m") != NULL);
	free(output);
...
static int test_editor_refresh_screen_drawer_header_mode_buttons(void) {
	...
	ASSERT_TRUE(strstr(output, TEST_DRAWER_COLLAPSE_CELL TEST_DRAWER_ACTIVE_EXPLORER_CELL
				...
				TEST_DRAWER_GIT_CELL TEST_DRAWER_MAIN_MENU_CELL) != NULL);
	free(output);
	...
	ASSERT_TRUE(strstr(output, TEST_DRAWER_COLLAPSE_CELL TEST_DRAWER_EXPLORER_CELL
				TEST_DRAWER_FILE_SEARCH_CELL TEST_DRAWER_PROJECT_SEARCH_CELL
				TEST_DRAWER_ACTIVE_GIT_CELL TEST_DRAWER_MAIN_MENU_CELL) != NULL);
	free(output);
	ASSERT_TRUE(editorDrawerGitToggle());
	...
	ASSERT_TRUE(strstr(output, TEST_DRAWER_COLLAPSE_CELL TEST_DRAWER_EXPLORER_CELL
				TEST_DRAWER_FILE_SEARCH_CELL TEST_DRAWER_PROJECT_SEARCH_CELL
				TEST_DRAWER_GIT_CELL TEST_DRAWER_ACTIVE_MAIN_MENU_CELL) != NULL);
	free(output);
